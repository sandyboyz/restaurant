...
class NavigationBar extends Component {
  ...
  render() {
    ...
    return (
      <nav className={`${classes["nav"]} ${!this.props.transparent? `bg-primary ${classes["navbar-dark"]}` : null}`}>
        ...
        <div className={`${classes["nav-items"]} ${this.state.isOpen? classes["nav-items-sidedrawer"] : null}`}>
          ...
          <Link className={classes["nav-item"]} to="/admin/"><i className={`fas fa-pencil-ruler ${classes["nav-icon"]} ${!this.props.transparent ? classes["nav-icon-transparent"] : null}`}></i>CRUD</Link>
